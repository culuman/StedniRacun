...
builder.Services.AddScoped<IRacunRepo>(provider =>
{
    var configuration = provider.GetRequiredService<IConfiguration>();
    var stringKonekcije = configuration.GetConnectionString("MojKonekcioniString");

    return new clsRacunRepo(stringKonekcije);
});
...
builder.Services.AddScoped<IUplataRepo>(provider =>
{
    var configuration = provider.GetRequiredService<IConfiguration>();
    var stringKonekcije = configuration.GetConnectionString("MojKonekcioniString");

    return new clsUplataRepo(stringKonekcije);
});

builder.Services.AddScoped<clsKorisnikServis>();
builder.Services.AddScoped<clsRacunServis>();
builder.Services.AddScoped<clsZahtevServis>();
builder.Services.AddScoped<clsUplataServis>();
